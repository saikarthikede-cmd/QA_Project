...
@dataclass
class VectorIndex:
    """LangChain vector store plus the chunks it was built from.

    Keeping the store around matters for app-level RAG flows: embeddings are
    still computed once at upload time, but retrieval no longer rebuilds a
    vector store for every generated FAQ or chat question.
    """
...
def build_vector_index(chunks: List[Chunk]) -> VectorIndex:
    """Build a reusable vector index from chunks that already have embeddings."""
    ...
    return VectorIndex(chunks=embedded, store=store)
...
def retrieve_from_index(
    ...
    if not index.chunks:
        return []
    return retrieve_from_index_with_embedding(embed_query(query), index, top_k)


def retrieve_from_index_with_embedding(
    ...
    results = index.store.similarity_search_with_score_by_vector(q_vec, k=top_k)
    return [
        RetrievedChunk(chunk=index.chunks[doc.metadata["_chunk_index"]], score=score)
...
def retrieve_with_embedding(
    ...
    if not index.chunks:
        return []
    return retrieve_from_index_with_embedding(query_embedding, index, top_k)
