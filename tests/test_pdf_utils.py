...
from shared.pdf_utils import build_vector_index, chunk_pages, embed_chunks, extract_pages, retrieve, retrieve_from_index
...
def test_reusable_vector_index_matches_retrieve(text_pdf):
    pdf_bytes = text_pdf(
        "Refunds are allowed within 30 days. "
        "API keys must be rotated every 90 days. "
        "Dispute evidence must be submitted within 7 days."
    )
    pages = extract_pages(pdf_bytes)
    chunks = embed_chunks(chunk_pages(pages))
    index = build_vector_index(chunks)

    one_shot = retrieve("API key rotation", chunks, top_k=2)
    indexed = retrieve_from_index("API key rotation", index, top_k=2)

    assert len(indexed) == len(one_shot)
    assert indexed[0].chunk.text == one_shot[0].chunk.text
