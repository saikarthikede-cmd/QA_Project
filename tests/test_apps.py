...
def test_app3_ask_agent_uses_search_tool(text_pdf, upload_pdf, monkeypatch):
    client = TestClient(app3_main.app)
    upload_pdf(client, "/upload")

    fake_client = FakeLLMClient(
        FakeResponse("", tool_calls=[make_tool_call("search_document", {"query": "shipping"})]),
        FakeResponse("Shipping is free over $50. Page 1."),
    )
    monkeypatch.setattr(app3_main, "validate_key", lambda provider, api_key: (fake_client, provider))
    assert client.post("/api/set-key", json={"provider": "groq", "api_key": "test-key"}).status_code == 200

    r = client.post("/ask", json={"question": "What is the shipping policy?"})
    assert r.status_code == 200
    data = r.json()
    assert data["answer"] == "Shipping is free over $50. Page 1."
    assert data["pages"] == [1]
    assert fake_client.call_count == 2, "ask should allow an agentic follow-up search before answering"


def test_app3_ask_answers_broad_edge_questions(text_pdf, monkeypatch, tmp_path):
    client = TestClient(app3_main.app)
    monkeypatch.setattr(app3_main, "CACHE_DIR", tmp_path)
    monkeypatch.setattr(app3_main, "CACHE_FILE", tmp_path / "last_upload.pkl")

    pdf_bytes = text_pdf(
        "Accounts not verified within 7 days are suspended. "
        "API keys must be rotated every 90 days. "
        "Full refunds within 30 days are processed at no fee. "
        "Chargeback evidence must be submitted within 7 days."
    )
    r = client.post("/upload", files={"file": ("smartpay.pdf", pdf_bytes, "application/pdf")})
    assert r.status_code == 200

    set_fake_key(monkeypatch, app3_main, client, FakeResponse("The document lists deadlines and consequences. Page 1."))
    r = client.post("/ask", json={"question": "What happens if someone violates the terms?"})
    assert r.status_code == 200
    assert r.json()["answer"] == "The document lists deadlines and consequences. Page 1."


def test_app3_small_manual_uses_full_context_for_in_document_questions(text_pdf, monkeypatch, tmp_path):
    client = TestClient(app3_main.app)
    monkeypatch.setattr(app3_main, "CACHE_DIR", tmp_path)
    monkeypatch.setattr(app3_main, "CACHE_FILE", tmp_path / "last_upload.pkl")

    pdf_bytes = text_pdf(
        "SmartPay is a cloud-based payment processing gateway. "
        "This manual covers setup, features, limits, fees, security, and troubleshooting. "
        "Accounts not verified within 7 days are suspended."
    )
    r = client.post("/upload", files={"file": ("smartpay.pdf", pdf_bytes, "application/pdf")})
    assert r.status_code == 200

    set_fake_key(monkeypatch, app3_main, client, FakeResponse("This manual explains SmartPay setup and rules. Page 1."))
    r = client.post("/ask", json={"question": "What is the main purpose of this document?"})
    assert r.status_code == 200
    assert r.json()["answer"] == "This manual explains SmartPay setup and rules. Page 1."

    r = client.post("/ask", json={"question": "What is the CEO's name?"})
    assert r.status_code == 200
    assert r.json()["answer"] == "No relevant content found in the document."


def test_app3_ask_restores_uploaded_pdf_after_reload(text_pdf, monkeypatch, tmp_path):
    client = TestClient(app3_main.app)
    monkeypatch.setattr(app3_main, "CACHE_DIR", tmp_path)
    monkeypatch.setattr(app3_main, "CACHE_FILE", tmp_path / "last_upload.pkl")

    pdf_bytes = text_pdf(
        "SmartPay supports credit cards, debit cards, digital wallets, and bank transfers. "
        "Standard settlement takes two business days."
    )
    r = client.post("/upload", files={"file": ("smartpay.pdf", pdf_bytes, "application/pdf")})
    assert r.status_code == 200

    app3_main._state.update({"chunks": [], "index": None, "filename": "", "faqs": []})

    set_fake_key(monkeypatch, app3_main, client, FakeResponse("SmartPay supports cards and bank transfers. Page 1."))
    r = client.post("/ask", json={"question": "What payment methods does SmartPay support?"})
    assert r.status_code == 200
    assert r.json()["answer"] == "SmartPay supports cards and bank transfers. Page 1."
    assert len(app3_main._state["chunks"]) > 0
